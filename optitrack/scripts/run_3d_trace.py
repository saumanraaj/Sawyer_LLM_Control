...
def run_3d_trace(args):
    ...
    # Per-body storage: {name: {xs, ys, zs, trail_line, current_dot, ghost_dot, smooth_pos}}
    tracked = {}
    color_counter = [0]
    ...
    def update(frame_num):
        ...
        for bname in names_to_track:
            ...
            # Simple exponential low-pass filter to visually smooth jitter.
            # Raw data is still logged; only the rendered point/trail are smoothed.
            # Higher alpha = less lag but more jitter. 0.2 = balanced, 1.0 = no smoothing.
            alpha = args.smooth
            raw_px, raw_py, raw_pz = float(pos[0]), float(pos[1]), float(pos[2])
            if bd["smooth_pos"] is None:
                px, py, pz = raw_px, raw_py, raw_pz
            else:
                spx, spy, spz = bd["smooth_pos"]
                px = alpha * raw_px + (1.0 - alpha) * spx
                py = alpha * raw_py + (1.0 - alpha) * spy
                pz = alpha * raw_pz + (1.0 - alpha) * spz
            bd["smooth_pos"] = (px, py, pz)

            # Skip drawing the first couple of seconds to avoid showing
            # the initial snapping while Motive locks onto the rigid body.
            if t_now < 2.0:
                continue
...
Examples:
  ...
  python run_3d_trace.py --demo --trail 5000 --rate 15            # longer trail
    ...
    parser.add_argument("--smooth", type=float, default=0.2,
                        help="Smoothing factor 0-1: higher = less lag, more jitter. "
                             "0.2=balanced, 0.05=very smooth/slow, 1.0=no smoothing (default: 0.2)")
